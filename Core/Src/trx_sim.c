...
/*uint8_t SIM_SendAtCommand(char *AtCommand, int32_t MaxWaiting_ms, char *Answer)
{
    ...
    {
        HAL_Delay(1000); // Remplace osDelay(1000)
        if ((HAL_GetTick() - start_time) >= MaxWaiting_ms)
        {
            return FAILURE; // Si le temps d'attente maximal est dépassé, retourner un échec
        }
    }
    ...
    {
        HAL_Delay(10); // Remplace osDelay(10)
...
void SIM_Init()
{
    ...
    HAL_Delay(200); // Remplace osDelay(200)
